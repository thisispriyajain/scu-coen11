...
//Saves content of list every 15 seconds
void *auto_save(void *arg) {
	FILE *file;
	NODE *temp;
	int i;
	...
	while(1) {
		...
		if((file = fopen((char*) arg, "wb")) == NULL) {
			printf("Error. Cannot load autosave file.\n");
		}
...
//Reads and prints content of binary file
void read_auto_saved(char *fileName) {
	FILE *file;
	NODE temp;
	pthread_mutex_lock(&lock);
